"""
SunFarm Smart Irrigation - Inference Engine & Farmer Advisor
============================================================
Provides a clean, modular prediction class that loads the trained lightweight
ML model and generates irrigation recommendations, confidence scores, and
human-readable farmer advice.
"""
...
class IrrigationPredictor:
    """
    Inference wrapper for the SunFarm ML irrigation recommendation model.
    """
    ...
    def validate_sensor_payload(self, sensor_data: Dict[str, Any]) -> Dict[str, float]:
        """
        Validates that all required sensor fields exist and conform to reasonable bounds.
        """
        cleaned = {}
        for feature in self.feature_names:
            ...
            bounds = SENSOR_BOUNDS.get(feature)
            if bounds:
                if val < bounds["min"] or val > bounds["max"]:
                    # Log warning or clip if minor, raise if dangerous
                    pass
    ...
    def generate_farmer_suggestion(
        ...
        """
        Generates actionable human-readable advisory text for farmers on the LCD display.
        """
...
def get_predictor() -> IrrigationPredictor:
    """Singleton getter for the IrrigationPredictor."""
...
def predict_irrigation(sensor_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Functional helper to predict irrigation requirement from raw sensor dict.
    """
